...
/* Macro Definitions                                      */
/**********************************************************/
#define HEATER_ON_WAIT_TIMEOUT					5//1 mins
...
UBYTE WaitForHeaterOn			 	=	HEATER_ON_WAIT_TIMEOUT; //30 sec waitime for turning on the heater to ensure the motor is running
	...
	if(IsHeaterTurnedOn())
	{
		...
		if(IsFanMotorRunning())
		{
			...
			if(ovenTemprature > configuredTemprature)
			{
				...
				//WaitForHeaterOn = HEATER_ON_WAIT_TIMEOUT;
